    ...
    <div class="forms-row">
      ...
      <section class="panel">
        ...
        <form @submit.prevent="saveCategory" class="vform">
          ...
          <label>Користувачі
            <select v-model="catForm.user_id" multiple size="4">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="c in categories" :key="c.id">
              <td>{{ c.title }}</td>
              <td>
                <button @click="editCategory(c)">✎</button>
...
const catForm  = ref({ id: null, title: '', user_id: [] });
...
function editUser(u) {
  userForm.value = { id: u.id, name: u.name, email: u.email };
...
async function saveCategory() {
  ...
  if (cid) {
    await api.put(`/categories/${cid}`, { title: catForm.value.title, user_id: catForm.value.user_id });
  } else {
    const res = await api.post('/categories', { title: catForm.value.title, user_id: catForm.value.user_id ?? null });
    cid = res.data.data.id;
  }
  // синхронізуємо користувачів → бекенд-ендпоінт PUT /categories/{id} { user_id }
  await api.put(`/categories/${cid}`, { user_id: catForm.value.user_id });

  catForm.value = { id: null, title: '', user_id: [] };
  await fetchCategories();
}
