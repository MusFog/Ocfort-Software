...
/* --- CRUD helpers --- */
async function fetchUsers()      { users.value      = (await api.get('/users')).data.data; }
async function fetchCategories() { categories.value = (await api.get('/categories')).data.data; }
...
onMounted(async () => {
  ...
  console.log(categories.value);   // тепер тут уже будуть дані
});
